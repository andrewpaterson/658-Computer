...
{
    auto mask = (uint16_t)(1 << bitNumber);
    *value = *value | mask;
}
...
bool bcdSum8Bit(uint8_t bcdFirst, uint8_t bcdSecond, uint8_t *result, bool carry) {
	...
	*result = 0;
	...
	while (shift < 8) {
        auto digitOfFirst = (uint8_t)(bcdFirst & 0xF);
        auto digitOfSecond = (uint8_t)(bcdSecond & 0xF);
        auto sumOfDigits = (uint8_t)(digitOfFirst + digitOfSecond + (carry ? 1 : 0));
		carry = sumOfDigits > 9;
...
bool bcdSubtract8Bit(uint8_t bcdFirst, uint8_t bcdSecond, uint8_t *result, bool borrow) {
	...
	*result = 0;
	...
	while (shift < 8) 
	{
        auto digitOfFirst = (uint8_t)(bcdFirst & 0xF);
        auto digitOfSecond = (uint8_t)(bcdSecond & 0xF);
        auto diffOfDigits = (uint8_t)(digitOfFirst - digitOfSecond - (borrow ? 1 : 0));
...
bool bcdSum16Bit(uint16_t bcdFirst, uint16_t bcdSecond, uint16_t *result, bool carry) {
	*result = 0;
	uint8_t shift = 0;
	while (shift < 16) {
        auto digitOfFirst = (uint8_t)(bcdFirst & 0xFF);
        auto digitOfSecond = (uint8_t)(bcdSecond & 0xFF);
