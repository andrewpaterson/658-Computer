...
namespace Binary 
{
	...
	{
		uint8_t mask = (uint8_t)(1 << bitNumber);
		*value = *value | mask;
	}
	...
	{
		uint8_t mask = (uint8_t)(1 << bitNumber);
		mask = (uint8_t)(mask ^ 0xFF);
		*value = *value & mask;
	}

	void setBitIn16BitValue(uint16_t *value, uint8_t bitNumber) 
	{
		uint16_t mask = (uint16_t)(1 << bitNumber);
		*value = *value | mask;
	}
	...
	bool bcdSum8Bit(uint8_t bcdFirst, uint8_t bcdSecond, uint8_t *result, bool carry) 
	{
		...
		*result = 0;
		...
		while (shift < 8) 
		{
			uint8_t digitOfFirst = (uint8_t)(bcdFirst & 0xF);
			uint8_t digitOfSecond = (uint8_t)(bcdSecond & 0xF);
			uint8_t sumOfDigits = (uint8_t)(digitOfFirst + digitOfSecond + (carry ? 1 : 0));
			carry = sumOfDigits > 9;
			if (carry) 
				sumOfDigits += 6;
			sumOfDigits &= 0xF;
	...
	bool bcdSubtract8Bit(uint8_t bcdFirst, uint8_t bcdSecond, uint8_t *result, bool borrow) 
	{
		...
		*result = 0;
		...
		while (shift < 8) 
		{
			uint8_t digitOfFirst = (uint8_t)(bcdFirst & 0xF);
			uint8_t digitOfSecond = (uint8_t)(bcdSecond & 0xF);
			uint8_t diffOfDigits = (uint8_t)(digitOfFirst - digitOfSecond - (borrow ? 1 : 0));
	...
	bool bcdSum16Bit(uint16_t bcdFirst, uint16_t bcdSecond, uint16_t *result, bool carry) 
	{
		*result = 0;
		...
		while (shift < 16) 
		{
			uint8_t digitOfFirst = (uint8_t)(bcdFirst & 0xFF);
			uint8_t digitOfSecond = (uint8_t)(bcdSecond & 0xFF);
