...
Address Address::sumOffsetToAddressNoWrapAround(const Address &address, uint16_t offset) 
{
    ...
    {
        ++newBank;
        newOffset = (uint16_t)(offsetSum - BANK_SIZE_BYTES);
    } 
    else 
    {
...
{
    int pageOfFirst = (int)std::floor(offsetFirst / PAGE_SIZE_BYTES);
    int pageOfSecond = (int)std::floor(offsetSecond / PAGE_SIZE_BYTES);
